	...
	generateTree(files, d) {
		...
		for (const npmKey in npmPaths) {
			...
			npmPaths[npmKey].forEach((npmPath) => {
				const path = this.processPath(npmPath, pathCache);
				nestedNpmPath.push(npmPath);
				tree[id].push(path);
			});
		}
		let res = null;
		if (depth < this.config.npmDepth) {
			res = this.generateTree(Object.values(nestedNpmPath), depth + 1);
		}
